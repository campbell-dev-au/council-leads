...
| `DevelopmentType[]` | Array of `{ "DevelopmentType": "<tag>" }`. One-to-many — an application can carry multiple tags. **Taxonomy differs by source, and also shifted over time within DA** (confirmed against real ingested data, not just samples): pre-2023 DA records use a coarse ~18-value taxonomy where pools are bundled into `Pools / decks / fencing` (27,891 records, also matches deck-only/fence-only work with no pool) — but from 2023 onward DA switched to a finer taxonomy with its own dedicated `Swimming pool` tag (10,921 records, zero overlap with `Pools / decks / fencing` — the two are never both applied to the same record), alongside separate `Balcony/deck/patio/terrace/verandah` and `Fences` tags. CDC uses an even finer ~238-value taxonomy that also went through the same pre-/post-2023 tag rename (`Swimming pools` → `Swimming pool`), plus a dedicated `Portable swimming pools and spas and child-resistant barriers` tag. This materially changes how precisely pool-related work can be identified — see §2.4. |
...
- There is **no free-text work description field** in either API — pool identification relies entirely on `DevelopmentType` tags, and precision **depends on both source and record age**. Measured against the real data now ingested: pre-2023 DA records only have the coarse `"Pools / decks / fencing"` tag (27,891 records, also matches deck-only/fence-only work with no pool); DA records from 2023 onward instead carry a dedicated `"Swimming pool"` tag (10,921 records) with no bundled deck/fence noise. CDC shows the same pre-/post-2023 split under a renamed tag — `"Swimming pools"` (17,342, pre-2023) and `"Swimming pool"` (13,927, 2023+) — plus a dedicated `"Portable swimming pools and spas and child-resistant barriers"` tag (607). **Recommendation for downstream lead classification: OR across all of `Pools / decks / fencing`, `Swimming pool`, `Swimming pools`, and the portable-pools tag to get full historical coverage across both sources and both taxonomy eras; treat only the pre-2023 `Pools / decks / fencing` matches as coarse/lower-confidence, since 2023+ tags on both DA and CDC are pool-specific.** Full detail and a recommended composite SQL filter (also covering cost outliers, application-type/status noise, and new-build-bundle exclusion) is in `POOL_LEAD_FILTERS.md`. This module makes no attempt to do that classification itself — it's noted here purely as a data-shape fact for whichever module does.
