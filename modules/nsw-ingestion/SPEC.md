...
| `DevelopmentType[]` | Array of `{ "DevelopmentType": "<tag>" }`. One-to-many — an application can carry multiple tags. **Taxonomy differs sharply by source** (confirmed against real ingested data, not just samples): DA uses a coarse ~18-value taxonomy (Dwelling, Dual occupancy, Multi-dwelling housing, Boarding house, Alterations and additions, Demolition, Change of use, Subdivision of land, **`Pools / decks / fencing`**, etc. — pools are bundled with decks/fencing, no dedicated tag). CDC uses a much finer ~238-value taxonomy that includes **dedicated pool tags**: `Swimming pool`, `Swimming pools`, and `Portable swimming pools and spas and child-resistant barriers`. This materially changes how precisely pool-related work can be identified — see §2.4. |
...
- There is **no free-text work description field** in either API — pool identification relies entirely on `DevelopmentType` tags, and precision **depends heavily on source**. Measured against the real data now ingested (§below): DA's only pool-adjacent tag is the coarse `"Pools / decks / fencing"` (~28k DA records, also matches deck-only/fence-only work with no pool). CDC, by contrast, has precise dedicated tags — `"Swimming pool"` (13,927), `"Swimming pools"` (17,342), and `"Portable swimming pools and spas and child-resistant barriers"` (607) — roughly 32k CDC records with high-confidence pool signal. **Recommendation for downstream lead classification: prefer CDC's dedicated pool tags as the primary signal; treat DA's `"Pools / decks / fencing"` as a coarse candidate filter only.** This module makes no attempt to do that classification itself — it's noted here purely as a data-shape fact for whichever module does.
